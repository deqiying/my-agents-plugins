...
1. Read [scoop.md](./references/scoop.md) for Windows Scoop work, including Scoop-owned `mise`.
2. Read [mise.md](./references/mise.md) for mise-managed runtimes and tools, project version mismatches, nested manager chains, or npm-global recovery after a Node change.
3. Read [brew.md](./references/brew.md) for macOS Homebrew and brew-owned tools.
4. Read [tool-registry.md](./references/tool-registry.md) together with [tool-registry.yaml](./references/tool-registry.yaml) when auditing or changing the shared desired tool set, manager ownership, version policy, or Node `postinstall` recovery list.
5. Read [placeholders.md](./references/placeholders.md) before writing persistent text that may contain local paths.
