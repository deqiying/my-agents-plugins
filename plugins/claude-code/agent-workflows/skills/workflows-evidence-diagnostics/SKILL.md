---
name: workflows-evidence-diagnostics
description: 用于重复失败、证据相互矛盾、source/generated/runtime 等状态可能不一致，或高回滚成本变更前的诊断与决策校验。通过区分状态、补充最少证据并在新证据出现时修正路径，避免凭惯性反复尝试；不用于常规多步骤任务或替代专用工具。
---
...
5. 若进入工程改动，使用 `$workflows-guidelines` 控制改动范围，并执行与风险相称的最小验证。

## 输出

- 仅报告结论、关键证据、已排除的状态与剩余未验证边界；不要公开冗长的内部推理。
- 当答复包含多个证据或决策时，使用 `$workflows-output-formatting`。
