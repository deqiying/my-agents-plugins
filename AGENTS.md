...
- When adding a new plugin directory, register that plugin in the relevant marketplace entry in the same change. For Codex, update `plugins/codex/.agents/plugins/marketplace.json`; when the repo-root marketplace is used for installation or compatibility, update `.agents/plugins/marketplace.json` too. Regenerate generated marketplaces such as `plugins/claude-code/.claude-plugin/marketplace.json` through the sync script instead of hand-editing them.
...
- Do not refresh the operator's local Codex, Claude Code, or other installed `my-agents-plugins` marketplace/cache state as part of ordinary repository edits. Limit routine work to repository sources, generated mirrors, validation, and optional commits. Run `codex plugin add`, `codex plugin marketplace add`, cachebuster/reinstall helpers, cache cleanup, or local product refresh flows only when the user explicitly asks for local installation testing or cache refresh.
