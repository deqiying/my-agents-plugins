...
- Name skills by the plugin's routing domain:
  ...
  - `onesearch` owns Onesearch CLI web research, provider bridge, and workflow bridge skills. Use `onesearch` for the main router skill and `onesearch-<route-id>` for bridge/workflow skills, such as `onesearch-search`, `onesearch-fetch`, `onesearch-deep-research`, `onesearch-context7`, `onesearch-exa`, `onesearch-tavily`, `onesearch-firecrawl`, `onesearch-deepwiki`, `onesearch-anysearch`, `onesearch-zhipu`, or `onesearch-docs`.
...
- Do not place Onesearch bridge skills under `tool-skills` or name them `tool-onesearch*` after the standalone `onesearch` plugin exists. Keep deprecated or removed upstream Onesearch routes out of the plugin unless the user explicitly asks for a compatibility shim.
